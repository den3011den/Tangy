...
namespace Tangy_Business.Repository
{
    public class ProductPriceRepository : IProductPriceRepository
    {
        ...
        {
            var objDromDb = await _db.ProductPrices.FirstOrDefaultAsync(u => u.Id == objDTO.Id);
            if(objDromDb!=null)
            {
                objDromDb.Price = objDTO.Price;
                objDromDb.Size = objDTO.Size;
                objDromDb.ProductId = objDTO.ProductId;
                _db.ProductPrices.Update(objDromDb);
                _db.SaveChanges();
                return _mapper.Map<ProductPrice, ProductPriceDTO>(objDromDb);
            }
            return objDTO;
        }
    }
